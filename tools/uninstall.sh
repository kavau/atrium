...
# Safety checks: warn about conditions that would leave the system broken.
# Skip with --force.
...
if [ -z "$DM_NAME" ] || [ "$DM_NAME" = "$SERVICE" ]; then
...
if [ "$FORCE" -ne 1 ] && { [ "$ATRIUM_ACTIVE" -eq 1 ] || [ "$NEEDS_ALTERNATIVE_DM" -eq 1 ]; }; then
    ...
    if [ "$ATRIUM_ACTIVE" -eq 1 ]; then
        echo "  - $SERVICE is currently running. Stopping it will end any graphical"
...
echo "Disabling $SERVICE service (if enabled)..."
...
echo "Removing state directory (if present)..."
if [ -d /var/lib/atrium ]; then
    rm -rf /var/lib/atrium
    echo "Removed /var/lib/atrium."
fi
